...
struct NetflixHomeView: View {
    ...
    private var backgroundGradientLayer: some View {
        ZStack {
            ...
            LinearGradient(
                colors: [
                    .netflixDrakRed.opacity(0.5),
                    .netflixDrakRed.opacity(0)
                ],
                startPoint: .top,
                endPoint: .bottom
            )
    ...
    private func heroCell(heroProduct: Product) -> some View {
        NetflixHeroCell(
            ...
            categories: [heroProduct.category?.capitalized ?? "Category", heroProduct.brand ?? "Brand"]) {
            ...
            ForEach(Array(productRows.enumerated()), id: \.offset) { (rowIndex, row) in
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ScrollView(.horizontal) {
                        ...
                        .padding(.horizontal, 16) // add padding here make sure the scroll still reach the edge of device
                    }
